  ...
  let t =
    create_obj ~in_db:true ~in_product_since:rel_rio ~in_oss_since:oss_since_303 ~internal_deprecated_since:None ~persist:PersistEverything ~gen_constructor_destructor:false ~name:_pif ~descr:"A physical network interface (note separate VLANs are represented as several PIFs)"
      ...
      ~contents:[ uid _pif;
                  ...
                  field ~in_oss_since:None ~ty:String ~in_product_since:rel_miami ~qualifier:DynamicRO "DNS" "Comma separated list of the IP addresses of the DNS servers to use" ~default_value:(Some (VString ""));
  ...
  let t =
    create_obj
      ...
      ~contents:[
        ...
        field ~qualifier:DynamicRO ~ty:(Set (Ref _vgpu_type)) ~lifecycle:[Published, rel_quebec, ""] ~ignore_foreign_key:true ~default_value:(Some (VSet [])) "compatible_types_in_vm" "List of VGPU types which are compatible in one VM";
        field ~qualifier:DynamicRO ~ty:(Set (Ref _vgpu_type)) ~lifecycle:[Published, rel_quebec, ""] ~ignore_foreign_key:true ~default_value:(Some (VSet [])) ~internal_only:true "compatible_types_on_pgpu" "List of VGPU types which are compatible on one PGPU";
      ]
      ()
end
    ...
    create_obj
      ...
      ~contents:
        [
          ...
          field ~qualifier:StaticRO ~ty:Float ~lifecycle:[Published, rel_stockholm, ""] "speed" "USB device speed" ~default_value:(Some (VFloat Constants.default_usb_speed));
        ]
      ~messages:
        [
          scan;
...
let emergency_calls =
  [ (Datamodel_pool.t,Datamodel_pool.slave_reset_master);
    ...
    (Datamodel_host.t,Datamodel_host.emergency_server_certificate_reset);
  ]
